...
public class ExportExcel {
    ...
    public void exportV2(List<?> objects, String[] headers, HttpServletResponse response){
        try {
            long startTime = System.currentTimeMillis();
            writeExcelV2(objects, headers, response);
            long endTime = System.currentTimeMillis();
            long timeExport = endTime - startTime;
            log.info("Time Export|" + timeExport);
        } catch (Exception e) {
    ...
    private static void writeExcelV2(List<?> objects, String[] headers, HttpServletResponse response) {
        try {
            // Create Workbook
            SXSSFWorkbook workbook = new SXSSFWorkbook();
            // Create sheet
    ...
    private static void writeObjectV2(Object object, SXSSFRow row) {
        Field[] fields = object.getClass().getDeclaredFields();
    ...
    public HttpServletResponse setResponse(HttpServletResponse response){
        try {
            ...
            String headerValue = "attachment; filename=report_cdn" + currentDateTime + ".xlsx";
    ...
    public void exportPaid(List<?> objects, String[] headers, HttpServletResponse response, String date){
        try {
            long startTime = System.currentTimeMillis();
            writeExcelPail(objects, headers, response, date);
            long endTime = System.currentTimeMillis();
            long timeExport = endTime - startTime;
            log.info("Time Export|" + timeExport);
        } catch (Exception e) {
    ...
    private static void writeExcelPail(List<?> objects, String[] headers, HttpServletResponse response, String date) {
        try {
            // Create Workbook
            SXSSFWorkbook workbook = new SXSSFWorkbook();
            // Create sheet
            ...
            titleCell.setCellValue("Paid Channel");
            ...
            // Write header
            writeHeaderPaid(sheet, rowIndex, headers, workbook);
            ...
            for (Object object : objects) {
                ...
                SXSSFRow row = sheet.createRow(rowIndex);
                // Write data on row
                writeObjectPaid(object, row, workbook);
                rowIndex++;
            }
            // Create file excel
            createOutputFilePaid(workbook, response);
            log.info("Done!!!");
        } catch (Exception e) {
            log.error("ERROR|" + e.getMessage(), e);
        }
    ...
    private static void writeHeaderPaid(SXSSFSheet sheet, int rowIndex, String[] headers, SXSSFWorkbook workbook) {
        // Create row
        SXSSFRow row = sheet.createRow(rowIndex);
        // Create cells
        SXSSFCell cell;

        CellStyle headerStyle = createHeaderStylePaid(workbook);
        for (int i = 0; i < headers.length; i++) {
            cell = row.createCell(i);
            cell.setCellValue(headers[i]);
    ...
    private static CellStyle createHeaderStylePaid(SXSSFWorkbook workbook) {
        CellStyle style = workbook.createCellStyle();
        Font font = workbook.createFont();
        font.setBold(true);
    ...
    private static void writeObjectPaid(Object object, SXSSFRow row, SXSSFWorkbook workbook) {
        Field[] fields = object.getClass().getDeclaredFields();
        for (int i = 0; i < fields.length; i++) {
            try {
                Field field = fields[i];
    ...
    private static void createOutputFilePaid(SXSSFWorkbook workbook, HttpServletResponse response) {
        try {
            ServletOutputStream outputStream = response.getOutputStream();
            workbook.write(outputStream);
            workbook.close();
